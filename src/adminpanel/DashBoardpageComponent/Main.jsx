  ...
  return (
    <div className="w-full relative">
      <div className="lg:h-[11.94%] max-lg:h-[240px]   flex max-lg:flex-col justify-between mb-5 flex-wrap items-center">
        {arr.map((data) => (
          <div className="bg-white shadow-md flex flex-col justify-between items-start p-4 max-lg:w-[40%] max-lg:h-[45%] rounded-lg ">
            <h3 className=" text-gray-500">{data.head}</h3>
            <p className="font-semibold text-2xl">{data.no}</p>
          </div>
        ))}
      </div>
      <div className="    flex flex-row max-lg:flex-col w-full max-lg:items-center bg-white lg:justify-between mb-6">
        <Tr />
        <TrafficSourceChart />
      </div>
      <div className="   max-lg:flex-col flex justify-between max-lg:items-center  ">
        <div className=" max-lg:m-3 w-[65%] max-lg:w-full rounded-lg shadow-lg border p-2 bg-white ">
          <TransactionTable />
        </div>
