...
if(!user)
  return <LoginPrompt title="Login to Apply for IdCard" />
if(data?.status==='pending')
  return <AlreadyApplied />
if(data?.status==='rejected')
  toast("Your Last Application has been rejected due to some reasons please apply again")
  return (
    <div>
    <div className="my-20 relative mx-32 flex flex-col gap-6 max-xl:mx-8 max-lg:mx-8 max-md:mx-4 max-sm:mx-2 ">
         {data?.status!=='approved' ?  <p className="text-xl font-medium">ID Card form</p> : <p className="text-xl font-medium">ID Card</p>}
          ...
          <div className="flex  w-full justify-between gap-16 max-md:flex-col-reverse  max-lg:gap-10 max-lg:">
            {
              user?.userType==='student' && data?.status!=='approved' && <IDform />
            }
              {
              user?.userType==='employee' && data?.status!=='approved' && <EmployeeForm />
            }
            {
              user?.userType==='student' ? <IdCardPurple  data={data}/> : <EmployeeIdCard data={data}/>
            }
          </div>
